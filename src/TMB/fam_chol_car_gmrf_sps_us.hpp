...
template<class Type>
  Type fam_chol_car_gmrf_sps_us(objective_function<Type>* obj)
{
  ...
  vector<Type> eta = X*beta + delta  + uc_par_l + uc_sib_l;
  ...
  //SparseMatrix<Type> diagI = getListElement(car_mats2,"diagI");
  //SparseMatrix<Type> N = getListElement(car_mats2,"N");
  //SparseMatrix<Type> Mpm5 = getListElement(car_mats2,"Mpm5");
  //SparseMatrix<Type> sigmainv =car_mats.Mpm5*(car_mats.diagI - rho*car_mats.N)*car_mats.Mpm5;
  //SparseMatrix<Type> sigmaSG = asSparseMatrix<Type>(Zs * (vc_s*invertSparseMatrix(sigmainv))* Zs.transpose()) ;
////  matrix<Type> gow = Zs * sigmaS * Zs.transpose() ;
  //Type gvc_s=0 ; 
  //for (int j=0; j < sigmaSG.rows(); j++){
  //  gvc_s +=sigmaSG.coeffRef(j,j)/(1.0*Zs.rows());
  // // gvc_s +=sigmaSG(j,j)/(1.0*Zs.rows());
  //}
  //Type gowtr = gow.trace();
  //Type nn = Zs.rows() * 1.0;
  //Type gvc_s = sigmaSG.trace() / (Zs.rows * 1.0);
  //---------------------------------------------
  if( binary_ind == 1){
  ...
  //Type Eb_gvc_s=khat + gvc_s/khat;

  Type tvc_pp=qnorm(1-Eb_pp);
  Type tvc_ss=qnorm(1-Eb_ss);
  //Type tvc_gvc_s=qnorm(1-Eb_gvc_s);
  ...
  //Type gvc_s_lia = (t - tvc_gvc_s*sqrt(1-(pow(t,2.0) - pow(tvc_gvc_s,2.0))*(1-t/i))) / (i + pow(tvc_gvc_s,2.0)*(i-t));
  
  Type vc_c_par_lia=vc_pp_adj; 
  Type vc_c_sib_lia=vc_ss_adj; 
