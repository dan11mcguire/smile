...
template<class Type>
  Type fam_chol_matern_gmrf_sgps_overpois(objective_function<Type>* obj)
{
  ...
  vector<Type> log_eta = X*beta + delta + ual + uc_par_l + uc_sib_l + res;
  ...
  for( int j=0; j< Za.rows(); j++){
    nll -= y(j)*log_eta(j) - exp(log_eta(j));
  } 
