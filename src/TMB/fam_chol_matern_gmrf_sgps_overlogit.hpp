...
template<class Type>
  Type fam_chol_matern_gmrf_sgps_overlogit(objective_function<Type>* obj)
{
  ...
  vector<Type> eta = X*beta + delta + ual + uc_par_l + uc_sib_l + res;
  ...
  //  nll -= dnorm( res(j) , Type(0.0), exp(log_sdvc_res), true );
  //} 
  for( int j=0; j< Za.rows(); j++){
   // nll -= dbinom(y(j), 1, invlogit(eta(j)), true);
