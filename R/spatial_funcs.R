...
gen_fam_ids<-function(n_trio = 5, n_quad = 5){

    id_dt<-data.table(set = rep(1:(n_trio + n_quad), times = rep(c(4, 3), times=c(n_quad, n_trio))),    
                      emprel = c(rep(c(1,2,3,3),times=n_quad), rep(c(1,3,3), times=n_trio)), 
                      ttype = rep(c("quad", "trio"), times = c(n_quad*4, n_trio*3)))

   # sstrio.sets <- id_dt[ttype == "trio", unique(set)]
   # ssquad.sets <- id_dt[ttype == "quad", unique(set)]
   #ss.emprel3.sets <- id_dt[,.(unique(set))][which(rbinom(id_dt[,uniqueN(set)], 1, 0.5) == 1)][,V1]
   #id_dt[(set %in% ss.emprel3.sets) & emprel == 3, `:=`(ss, 1L)]
   #id_dt[(!set %in% ss.emprel3.sets) & emprel == 3, `:=`(ss, 0L)]
   #id_dt[emprel != 3, `:=`(ss, 0L)]
   
    id_dt[, `:=`(pid, 1:.N)]
#    setkeyv(id_dt, c("ttype", "set", "emprel","pid"))
#    setorderv(id_dt, c("ttype", "set", "emprel","pid"), c(1, 1, 1, 1))
